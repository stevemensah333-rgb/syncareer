...
/**
 * Mentor directory: Connect with verified professionals for focused, application-relevant guidance.
...
export default function MentorDirectory() {
  ...
  useEffect(() => {
    mentorshipApi
      .listMentors()
      .then(setMentors)
      .catch(() => setError('Mentors could not be loaded.'))
  ...
  return (
    <PageLayout
      title="Find a mentor"
      description="Connect with verified professionals for focused, application-relevant guidance."
    >
      <div className="space-y-6">
        ...
        <div className="surface-content grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-[minmax(220px,1fr)_170px_170px_170px_170px]">
          <div className="relative">
            ...
            <Input
              ...
              className="pl-9"
              placeholder="Name, role, company or skill"
              value={search}
              onChange={(e) => setFilter('q', e.target.value)}
            />
          </div>
          <Select value={experience} onValueChange={(v) => setFilter('experience', v)}>
          ...
          <Select value={role} onValueChange={(v) => setFilter('role', v)}>
          ...
          <Select value={availability} onValueChange={(v) => setFilter('availability', v)}>
            <SelectTrigger aria-label="Availability">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All availability</SelectItem>
              <SelectItem value="accepting">Accepting requests</SelectItem>
              <SelectItem value="limited">Limited availability</SelectItem>
            </SelectContent>
          </Select>
          <Select value={skill} onValueChange={(v) => setFilter('skill', v)}>
            <SelectTrigger aria-label="Expertise">
        ...
        {loading && (
          <div className="space-y-2" aria-busy="true" role="status">
            {[1, 2, 3].map((n) => (
              <div key={n} className="h-16 animate-pulse border border-border bg-muted/40 motion-reduce:animate-none" />
            ))}
          </div>
        )}
          ...
          <RecordState
            ...
            action={
              <Button
                ...
                variant="outline"
                onClick={() => {
                  for (const key of ['q', 'experience', 'role', 'availability', 'skill']) setFilter(key, 'all');
                }}
              >
                Clear filters
              </Button>
          ...
          <RecordList label="Verified mentors">
            {filtered.map((mentor) => (
              <RecordRow
                ...
                onClick={() =>
                  navigate(
                    `/mentors/${mentor.mentor_id}${applicationParam ? `?application=${encodeURIComponent(applicationParam)}` : ''}`
                  )
                }
              />
            ))}
        ...
        {/* Intentional Lower Section: How Mentorship Works & Verification Note */}
        <div className="rounded-surface border border-border/70 bg-card p-5 sm:p-6 space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Targeted, asynchronous career advice from verified industry professionals
              </p>
            </div>
            <p className="text-xs text-muted-foreground flex items-center gap-1">
              <Users aria-hidden="true" className="h-3.5 w-3.5 text-muted-foreground" />
              Company email domain verified
            </p>
          </div>

          <div className="grid gap-3 sm:grid-cols-3 pt-1">
            <div className="rounded-surface border border-border bg-secondary/20 p-3.5 space-y-1">
              <div className="flex items-center gap-1.5 text-xs font-semibold text-foreground">
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-primary/10 text-[10px] text-primary font-bold">1</span>
                Submit Focused Request
              </div>
              <p className="text-[11px] text-muted-foreground leading-relaxed">
                Connect your request to a specific application or CV with a clear, actionable goal.
              </p>
            </div>

            <div className="rounded-surface border border-border bg-secondary/20 p-3.5 space-y-1">
              <div className="flex items-center gap-1.5 text-xs font-semibold text-foreground">
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-primary/10 text-[10px] text-primary font-bold">2</span>
                Mentor Context Review
              </div>
              <p className="text-[11px] text-muted-foreground leading-relaxed">
                Mentors review your goal and role context before accepting to ensure they can provide real help.
              </p>
            </div>

            <div className="rounded-surface border border-border bg-secondary/20 p-3.5 space-y-1">
              <div className="flex items-center gap-1.5 text-xs font-semibold text-foreground">
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-primary/10 text-[10px] text-primary font-bold">3</span>
                Direct Communication
              </div>
              <p className="text-[11px] text-muted-foreground leading-relaxed">
                Upon acceptance, verified contact emails are exchanged to continue via email or scheduling.
              </p>
            </div>
          </div>
        </div>
