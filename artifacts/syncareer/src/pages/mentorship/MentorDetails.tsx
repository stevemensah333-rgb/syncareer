...
export default function MentorDetails() {
  ...
  return <PageLayout title={mentor.full_name} description={`${mentor.current_role ?? 'Career mentor'} · ${mentor.company_name}`}>
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
      <Card><CardContent className="space-y-6 p-6"><div className="flex gap-4"><Avatar className="h-20 w-20"><AvatarImage src={mentor.avatar_url ?? ''} /><AvatarFallback className="text-xl">{mentor.full_name.charAt(0)}</AvatarFallback></Avatar><div><h2 className="text-2xl font-semibold">{mentor.full_name}</h2><p className="text-muted-foreground">{mentor.current_role}</p><p className="mt-1 flex items-center gap-1 text-sm text-primary"><CheckCircle2 className="h-4 w-4" />Company email verified · {mentor.company_name}</p></div></div><div><h2 className="font-semibold">About</h2><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-muted-foreground">{mentor.bio}</p></div><div><h2 className="font-semibold">Expertise</h2><div className="mt-2 flex flex-wrap gap-2">{mentor.expertise_tags.map((tag) => <Badge key={tag} variant="secondary">{tag}</Badge>)}</div></div><p className="text-sm text-muted-foreground">{mentor.years_experience} years of experience · {mentor.availability_status === 'accepting' ? 'Accepting requests' : 'Limited availability'}</p></CardContent></Card>
      <Card><CardHeader><CardTitle>Request mentorship</CardTitle></CardHeader><CardContent><form className="space-y-4" onSubmit={submit}><div className="space-y-2"><Label htmlFor="request-type">Request type</Label><Select value={requestType} onValueChange={(v) => setRequestType(v as MentorshipRequestType)}><SelectTrigger id="request-type"><SelectValue /></SelectTrigger><SelectContent>{Object.entries(MENTORSHIP_REQUEST_TYPES).map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label htmlFor="request-goal">What outcome do you want?</Label><Input id="request-goal" maxLength={240} value={goal} onChange={(e) => setGoal(e.target.value)} placeholder="e.g. Identify the strongest improvements to my CV" /></div><div className="space-y-2"><Label htmlFor="request-context">Context</Label><Textarea id="request-context" rows={5} maxLength={2000} value={context} onChange={(e) => setContext(e.target.value)} placeholder="Explain the role, challenge and guidance you need." /></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="request-deadline">Deadline</Label><Input id="request-deadline" type="date" min={new Date().toISOString().slice(0,10)} value={deadline} onChange={(e) => setDeadline(e.target.value)} /></div><div className="space-y-2"><Label htmlFor="request-url">Supporting URL</Label><Input id="request-url" type="url" maxLength={500} value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://" /></div></div><div className="space-y-2"><Label htmlFor="request-application">Application context</Label><Select value={applicationId} onValueChange={setApplicationId}><SelectTrigger id="request-application"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">No application</SelectItem>{resources.applications.map((item) => <SelectItem key={item.id} value={item.id}>{item.label}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label htmlFor="request-cv">CV</Label><Select value={resumeId} onValueChange={setResumeId}><SelectTrigger id="request-cv"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">No CV</SelectItem>{resources.resumes.map((item) => <SelectItem key={item.id} value={item.id}>{item.label}</SelectItem>)}</SelectContent></Select><p className="text-xs text-muted-foreground">The mentor sees only the CV title until accepting.</p></div><div className="flex items-start gap-2 rounded-md border p-3 text-sm"><Checkbox id="request-consent" checked={consent} onCheckedChange={(checked) => setConsent(checked === true)} /><Label htmlFor="request-consent" className="font-normal leading-5">I agree that our account emails will be exchanged if this request is accepted. A selected CV becomes readable by this mentor only after acceptance.</Label></div><Button type="submit" className="w-full" disabled={!consent || submitting}>{submitting && <Loader2 className="h-4 w-4 animate-spin" />}Send request</Button></form></CardContent></Card>
    </div>
  </PageLayout>;
}
