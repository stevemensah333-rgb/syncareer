...
function computeStrengths(result: AssessmentResult): string[] {
  const strengths: string[] = [];
  const riasec = result.work_interest_score_json;
  const highScores: [string, number][] = Object.entries(riasec).filter(([ , score]) => score >= 80) as [string, number][];
  highScores.sort((a, b) => b[1] - a[1]);
  for (const [code, score] of highScores) {
    const label = RIASEC_LABELS[code] || code;
    strengths.push(`${label} interest (score: ${score})`);
  }
  const personality = result.personality_score_json;
  const personalityKeys = Object.keys(personality).sort((a, b) => personality[b]! - personality[a]!);
  if (personalityKeys.length > 0) {
    strengths.push(`Strong ${personalityKeys[0]} tendency (question responses)`);
  }
  const skills = result.skills_score_json;
  const skillsKeys = Object.keys(skills).sort((a, b) => skills[b]! - skills[a]!);
  if (skillsKeys.length > 0) {
    strengths.push(`Prefers ${skillsKeys[0]}-type tasks (question responses)`);
  }
  if (strengths.length === 0) {
    strengths.push('Your engagement across all assessment areas');
  }
  return strengths;
}

function computeGaps(result: AssessmentResult): string[] {
  const gaps: string[] = [];
  const riasec = result.work_interest_score_json;
  const lowScores: [string, number][] = Object.entries(riasec).filter(([ , score]) => score <= 30) as [string, number][];
  lowScores.sort((a, b) => a[1] - b[1]);
  for (const [code, score] of lowScores) {
    const label = RIASEC_LABELS[code] || code;
    gaps.push(`Develop ${label}-related skills (current score: ${score})`);
  }
  if (gaps.length === 0) {
    gaps.push('Broaden your experience in unfamiliar work themes');
  }
  return gaps;
}

function determineNextAction(result: AssessmentResult): { key: string; label: string; badge: string; description: string; evidenceSuggestions?: string[]; opportunities?: { title: string; url: string }[] } {
  const primary = result.primary_interest;
  const riasecScores = result.work_interest_score_json;
  const sorted = Object.entries(riasecScores).sort((a, b) => b[1] - a[1]);
  const dominant = sorted[0];
  const dominantCode = dominant?.[0] ?? 'C';
  const dominantScore = dominant?.[1] ?? 0;
  const themeLabels: Record<string, { label: string; badge: string }> = {
    R: { label: 'Explore Hands-On Roles', badge: 'Practical' },
    I: { label: 'Investigate Analytical Paths', badge: 'Investigative' },
    A: { label: 'Express Creative Strengths', badge: 'Artistic' },
    S: { label: 'Connect with People-Focused Work', badge: 'Social' },
    E: { label: 'Lead with Enterprise Drive', badge: 'Enterprising' },
    C: { label: 'Organize with Structure', badge: 'Conventional' },
  };
  const theme = themeLabels[dominantCode] || { label: 'Explore Opportunities', badge: 'Discover' };
  const evidenceSuggestions: string[] = [];
  const lowScores = Object.entries(riasecScores).filter(([ , score]) => score <= 40);
  if (lowScores.length > 0) {
    for (const [code, score] of lowScores) {
      const label = RIASEC_LABELS[code] || code;
      evidenceSuggestions.push(`Gain exposure to ${label.toLowerCase()} environments`);
    }
  }
  if (evidenceSuggestions.length === 0) {
    evidenceSuggestions.push('Continue developing your strongest interests');
  }
  const opportunities = [
    { title: `${theme.label} Career Paths`, url: `/opportunities?q${ dominantCode }` },
  ];
  return {
    key: dominantCode,
    label: theme.label,
    badge: theme.badge,
    description: `Your dominant RIASEC theme is ${primary} (score: ${dominantScore}%). This represents your natural interest area.`,
    evidenceSuggestions,
    opportunities,
  };
}
