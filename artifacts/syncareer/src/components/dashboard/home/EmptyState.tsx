...
export function EmptyState({ hasApplicationHistory = false, showAssessment = false }: { hasApplicationHistory?: boolean; showAssessment?: boolean }) {
  ...
  return (
    <section className="surface-content" aria-labelledby="application-desk-empty-title">
      <div className="grid border-b border-border md:grid-cols-[8rem_1fr]">
        <div className="border-b border-border bg-muted/40 px-4 py-5 md:border-b-0 md:border-r">
          <p className="type-label">Desk</p>
        </div>
        <div className="p-6 md:p-10">
        <div className="max-w-2xl space-y-5">
          <div className="space-y-3">
            <p className="type-label text-primary">Your next piece of work</p>
            <h2 id="application-desk-empty-title" className="type-section-title text-xl md:text-2xl">{hasApplicationHistory ? 'Choose the next opportunity to pursue' : 'Start with a real opportunity'}</h2>
