      ...
      <CardContent className="space-y-5">
        <section className="space-y-3" aria-labelledby="cv-completion-heading">
          <div className="flex items-baseline justify-between gap-3">
            <div>
              <h3 id="cv-completion-heading" className="text-sm font-semibold">Completion</h3>
              <p className="text-xs text-muted-foreground">Meaningful information added</p>
            </div>
            <span className="text-2xl font-bold tabular-nums" aria-label={`${completion.percentage}% complete`}>
        ...
        <section className="space-y-3" aria-labelledby="cv-quality-heading">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h3 id="cv-quality-heading" className="text-sm font-semibold">CV quality</h3>
          ...
          <div className="space-y-2">
            {Object.entries(breakdown).map(([key, category]) => (
              <div key={key} className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium tabular-nums">{category.score}/{category.max}</span>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-2">
            ...
            <ul className="space-y-1.5">
              {strengths.map((strength) => (
                <li key={strength} className="relative pl-4 text-xs leading-relaxed text-muted-foreground before:absolute before:left-0 before:top-1.5 before:h-1.5 before:w-1.5 before:rounded-full before:bg-success">
                  {strength}
                </li>
              ))}
            </ul>
          ...
          <div className="space-y-2">
            ...
            <ul className="space-y-1.5">
              {suggestions.map((suggestion) => (
                <li key={suggestion} className="relative pl-4 text-xs leading-relaxed text-muted-foreground before:absolute before:left-0 before:top-1.5 before:h-1.5 before:w-1.5 before:rounded-full before:bg-warning">
                  {suggestion}
                </li>
              ))}
            </ul>
