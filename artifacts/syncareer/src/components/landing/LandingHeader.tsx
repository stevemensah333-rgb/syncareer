...
export default function LandingHeader({ onSignIn, onSignUp }: LandingHeaderProps) {
  ...
  const NavItem = ({ icon: Icon, label, onClick }: { icon: typeof Home; label: string; onClick: () => void }) => (
    <button
      onClick={onClick}
  ...
  return (
    <header className="fixed top-4 left-0 right-0 z-50 flex justify-center px-4 pointer-events-none">
      <div className="pointer-events-auto flex items-center gap-1 pl-2 pr-2 py-1.5 rounded-full bg-black/85 backdrop-blur-xl border border-white/10 shadow-lg">
