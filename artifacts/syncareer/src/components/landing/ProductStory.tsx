...
export default function ProductStory() {
  ...
  return (
    <>
      {/* Section 1: Six connected workflow steps */}
      <AnimatedSection>
            ...
            <ol className="mt-10 grid overflow-hidden rounded-surface border sm:grid-cols-2 lg:grid-cols-6">
              {APPLICATION_STAGES.map((stage, index) => (
                <li key={stage.id} className="relative border-b last:border-b-0 sm:border-r sm:[&:nth-child(2n)]:border-r-0 lg:border-b-0 lg:[&:nth-child(2n)]:border-r lg:last:border-r-0">
                  <button
                    type="button"
                    onClick={() => openHeroStage(stage.id)}
                    className="group interactive block min-h-28 w-full p-5 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring"
                  >
                    <span className="inline-flex h-6 min-w-6 items-center justify-center rounded-md bg-primary/10 px-1.5 text-xs font-semibold text-primary transition-colors duration-150 group-hover:bg-primary group-hover:text-primary-foreground">0{index + 1}</span>
                    <span className="mt-3 block text-sm font-semibold transition-colors duration-150 group-hover:text-primary">{stage.label}</span>
      ...
      {/* Section 2: Concrete evidence transformation */}
          ...
          <div className="mx-auto w-full max-w-[1280px] px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
            ...
            <div className="mt-8 grid gap-px overflow-hidden rounded-surface border bg-border sm:grid-cols-2 lg:grid-cols-4">
              {trust.map(({ icon: Icon, title, text }) => (
                <div key={title} className="group bg-card p-5 transition-colors duration-150 hover:bg-secondary/50">
                  <span className="grid h-9 w-9 place-items-center rounded-md bg-primary/10 text-primary transition-colors duration-150 group-hover:bg-primary group-hover:text-primary-foreground">
...
function TransformationRow({
  ...
  return (
    <div
      className={`group p-4 sm:p-5 transition-colors duration-150 hover:bg-secondary/30 ${
        bordered ? "border-t sm:border-l sm:border-t-0" : "border-b"
      }`}
    >
