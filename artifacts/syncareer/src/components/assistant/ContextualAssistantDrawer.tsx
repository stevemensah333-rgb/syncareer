    ...
    <Sheet>
      ...
      <SheetContent side="right" className="flex w-full flex-col sm:max-w-lg">
        ...
        <div className="mt-4 flex-1 space-y-5 overflow-y-auto pr-1">
          ...
          <div className="space-y-2"><Label htmlFor={`assistant-${task}`}>What should the assistant help with?</Label><Textarea id={`assistant-${task}`} value={instruction} onChange={(event) => setInstruction(event.target.value)} rows={5} /><Button onClick={() => void send()} disabled={pending || !instruction.trim() || selected.length === 0}>{pending ? <Spinner className="size-4" /> : <Send className="h-4 w-4" />}{pending ? 'Preparing proposal…' : 'Request proposal'}</Button></div>
