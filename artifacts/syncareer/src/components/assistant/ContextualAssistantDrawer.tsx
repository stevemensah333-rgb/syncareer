    ...
    <Sheet>
      ...
      <SheetContent side="right" className="flex w-full flex-col sm:max-w-lg" aria-describedby="assistant-description">
        ...
        <div className="mt-4 flex-1 space-y-5 overflow-y-auto pr-1">
          ...
          {proposal && <section className="rounded-lg border bg-card p-4" aria-labelledby="proposal-heading"><h3 id="proposal-heading" className="font-medium">Proposal</h3><p className="mt-2 whitespace-pre-wrap text-sm">{proposal.text}</p><p className="mt-3 text-xs text-muted-foreground">Review this against your source material. Nothing changes automatically.</p><div className="mt-3 flex flex-wrap gap-2">{onAccept && <Button size="sm" onClick={() => { try { onAccept(proposal.text); emitDecided('accepted'); } catch { } setAccepted(proposal); setProposal(null); }}><Check className="h-4 w-4" />{acceptLabel}</Button>}<Button size="sm" variant="outline" onClick={() => { setRejected(proposal); setProposal(null); emitDecided('rejected'); }}>Reject</Button></div></section>}
