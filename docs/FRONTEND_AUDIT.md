...
| Profile/Settings | Real, section-scoped (2026-09-04) | `pages/Settings.tsx` is a shell over `components/settings/*`: Account (Profile, Account & Security) · Preferences (Notifications, Preferences) · Support (Feedback, Help, optional Support Syncareer). Identity fields are editable where RLS allows (name/username/summary/LinkedIn), education and qualifications keep their own editors, email/password/other-sessions use `supabase.auth`, closure uses the `delete-account` seam. No geo-IP lookup, no compact-view switch, no subscription page. |
...
| i18n | Incomplete | Config and 9 non-English locale files exist, but most are stubs and only Settings consumes translations (`Settings.tsx`, `PreferencesSection`, `AccountSecuritySection`). Settings now offers exactly `SUPPORTED_LANGUAGES` from `i18n/config.ts`, so a selection cannot silently fall back to English; browser detection can still produce mixed-language Settings. |
...
- **Profile/settings (delivered 2026-09-04):** one shell + one component per
  destination — `ProfileSection` (identity editor + `EducationSection` +
  `QualificationsSection`), `AccountSecuritySection`, `NotificationsSection`,
  `PreferencesSection`, `FeedbackSection`, `HelpSection`, `SupportSection`, all
  composed from `SettingsScaffold` groups/rows so no destination invents its own
  card. Editable/read-only states are explicit per row; there is no
  `SubscriptionSummary` and no billing surface.
...
| Profile/settings | `pages/Settings.tsx`, `components/settings/{SettingsScaffold,ProfileSection,EducationSection,QualificationsSection,AccountSecuritySection,NotificationsSection,PreferencesSection,FeedbackSection,HelpSection,SupportSection}.tsx`, `lib/{displayPreferences,localePreferences,contact,support,accountRoles}.ts`, `hooks/useNotificationPreferences.ts`, `contexts/UserProfileContext.tsx`, `utils/languages.ts` | Read-only/editable contract, i18n decision and theme/device-preference behavior are resolved (2026-09-04). Do not weaken delete-account or auth boundaries; do not add a control the backend has no API for (session lists, avatar upload, 2FA, data export). |
