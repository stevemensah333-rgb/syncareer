...
3. The app calls deployed-only functions: `send-notification`, `admin-feedback`, `admin-users`, `delete-account`, `check-feature-access`, `compute-user-intelligence`, `interview-tts`, `mock-interview`, `analyze-portfolio`, `verify-paystack-payment`, `compute-university-insights`, and `cv-ai-assistant`. Their current deployed source/config is not in the repository. Recover exact source from Lovable Cloud before changing contracts.
...
7. AI inputs include CV content, interview answers, profile context, and chat. Preserve the authenticated function boundaries and cost/usage enforcement. The generic SynAI client currently sends context directly to the `career-guidance` function; do not broaden it into a generic AI feature.
