...
| 1.3 | Product model: feature access does not depend on subscription/plan state; account menu offers Feedback and no Subscription/Upgrade/Billing items | `Navbar.test.tsx`, `ApplicationInterview.test.tsx` | Replaces the former `featureAccess.test.ts` premium-gating coverage; menu Feedback/Help now route into Settings (1.25) |
...
| 1.23 | Market Intelligence derivation: word-boundary skill matching (no "java"→"javascript" false positives), demand direction from forecast, common/emerging/declining requirement classification, gap ranking with live-posting evidence, the one-paragraph conclusion, active-application counting, and local-salary/date formatting | `features/market-intelligence/derive.test.ts` | Pure functions; no DB | 
| 1.24 | Market Intelligence page (decision-support): opens with the conclusion + provenance (source, methodology, freshness, "AI estimate" markers), personalisation from recorded skills/applications, position/gaps/next-actions sections, no fabricated readiness scores, no-major prompt, retryable error, loading skeleton, and partial-personalisation warning | `pages/Analysis.test.tsx`, `hooks/useMarketSignals.test.ts` | happy-dom; mocked `useMarketIntelligence`/`useMarketSignals`/`useUserProfile` | 

| 1.25 | Settings (administrative surface): grouped destinations (Account → Profile, Account & Security · Preferences → Notifications, Preferences · Support → Feedback, Help) with no Regional or Subscription page, `?tab=` deep links plus `security`/`regional`/`subscription` aliases, mobile list → section with back control, identity save limited to owner-writable `profiles` columns with a context refresh, education/qualification editors scoped to their own tables, notification switches bound to real `notification_preferences` columns, device-local language/region/time zone/theme with no geo-IP lookup, feedback written to `user_feedback` with its kind, and the optional Support destination hidden until `VITE_SUPPORT_URL` is set | `pages/Settings.test.tsx` | happy-dom; mocked Supabase/auth/profile context; deployed `delete-account` remains Layer 4.3 |
