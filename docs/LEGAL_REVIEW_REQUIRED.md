...
This is a product/repository reconciliation record, not legal advice. On 2026-09-04 the monetization clauses were updated to the free-product model (see rows below) as part of the approved product change; the changes still need owner/counsel sign-off, and the effective date was intentionally not changed.
...
| Subscription entry points | SUPERSEDED 2026-09-04 | Former `PaystackButton.tsx`, `Pricing.tsx`, `SubscriptionManager.tsx` and `/subscription-success` were removed; `/pricing` and `/subscription-success` now redirect to `/`. Optional one-time support is hidden unless `VITE_SUPPORT_URL` is configured (currently unset). |
...
| Free Service & Support (formerly Payments & Subscriptions) | UPDATED — REVIEW REQUIRED | New wording: Syncareer is free to use; no features gated by payment; optional voluntary one-time support never unlocks functionality. | Pricing/refund claims removed from Terms and Privacy. Effective date not changed pending owner/counsel sign-off. If `VITE_SUPPORT_URL` is later configured, its hosted provider terms must be disclosed. | Approve free-service wording and the optional-support disclosure (and set the support destination deliberately). | Yes | High |
...
- The `/pricing` refund/payment-method claims are gone with the page; no new monetization copy should be reintroduced. If the optional one-time support link is enabled later, confirm the hosted provider's payment disclosures with counsel.
